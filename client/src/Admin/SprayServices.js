...
//let tableBody;
const getSprayServices = async()=>{

     await axios
      .get(API+"/sprayservicesenquiry")
      .then((response) => {
        ...
        //else{alert(`The result for the search value ${search} is not found`)}
          ...
          .map((val,index)=>{
            ...
            <td className="td">{val.place}</td>
            <td className="td">{val.crop}</td>
            <td className="td">{val.acre}acre</td>
            <td className="td">{val.message}</td>
...
const Id = id;
   axios.delete(API+`/sprayservicesenquiry/${Id}`)
                .then((response) => {
...
<div style={{width:"100%"}}>
    <AdminAppbar title={"SprayServices Booked"}/>
    </div><br />
<section>
        ...
        <Grid    columnSpacing={{ xs: 1, sm: 2, md: 3,large:2 }} className="container1" style={{width:"100%"}}>
              ...
              <Grid item xs={12} style={{width:"100%",paddingLeft:"4px"}}>
                ...
                {    
                  ...
                  //                                        <input type="submit"  className="quiry-btn" value="Search" onClick={getSprayServices} ></input></span></div>
                                                        }                      
                   ...
                   <span  style={{float:"right",marginRight:"25px",cursor:"pointer",}} >
                    ...
                    className="quiry-btn"
                    table="table"
                    filename="SprayServices-Booked-List"
                    sheet="SprayServices-Booked-List"
                    buttonText="Download"
    ...
    <table ref={tableRef} style={{border:"solid green 2px"}} id="table">
                <thead style={{}}>
                  <tr style={{}}>
                    ...
                    <th scope="col" className="th"><h6>Place</h6></th>
                    <th scope="col" className="th"><h6>Crop</h6></th>
                    <th scope="col" className="th"><h6>Land</h6></th>
